...
void PredictVideo(const std::string& video_path,
  ...
  while (capture.read(frame)) {
    ...
    for (const auto& item : result) {
      ...
      if (item.rect.size() > 6) {
        ...
        printf("class=%d confidence=%.4f rect=[%d %d %d %d]\n",
               ...
               item.rect[2],
               item.rect[3]);
        tactile_paving::signalLamp signalLamp_msg;
        signalLamp_msg.lamp_class = labels[item.class_id];
        signalLamp_msg.xmin = (item.rect[0]<0)?0:item.rect[0];
        signalLamp_msg.ymin = (item.rect[1]<0)?0:item.rect[1];
        signalLamp_msg.xmax = (item.rect[2]<0)?0:item.rect[2];
        signalLamp_msg.ymax = (item.rect[3]<0)?0:item.rect[3];
        signalLamp_msg.fps = frame_id;
    ...
    cv::Mat image_resize;
    cv::resize(out_im,image_resize,cv::Size(320,320));
    cv::imshow("paddli_main",image_resize);
...
std::string getIMG_dir(std::string cam_up,std::string IpLastSegment){
  ...
  std::string udpSendIntegratedPipe = udpstrPrevData +  std::to_string(udpPORT[stoi(cam_up)-1]) + udpstrBehindData;
  std::cout<<"udpSendIntegratedPipe:"<<udpSendIntegratedPipe<<std::endl;
  return udpSendIntegratedPipe;
}

int main(int argc, char** argv) {
  // Parsing command-line
  ros::init(argc, argv, "paddle_detecion_main");
  ros::NodeHandle nh;
  turtle_vel_pub =  nh.advertise<tactile_paving::signalLamp>("/signalLamp",10);

  std::string cam_up;
  std::string IpLastSegment;
  std::string paddle_model_path;

  nh.param<std::string>("/cam_up",cam_up,"0");
  nh.param<std::string>("/IpLastSegment",IpLastSegment,"15");
  nh.param<std::string>("/paddle_model_path",paddle_model_path,"");  

  google::ParseCommandLineFlags(&argc, &argv, true);
  FLAGS_model_dir = paddle_model_path;
  // FLAGS_video_file = getIMG_dir(cam_up,IpLastSegment);
  FLAGS_video_file = "/home/lhy/C715228BBC959C4D5927324853D6DA5D.mp4";
  if (FLAGS_model_dir.empty() ||
      (FLAGS_image_file.empty() && FLAGS_image_dir.empty() &&
       FLAGS_video_file.empty())) {
    std::cout << "error:请检查参数问题" << std::endl;
    return -1;
  }
